package a.gatekeeper.util;

import java.util.Arrays;

public class ByteOps
{
  public static byte[] xor( byte[] a, byte[] b )
  {
    if ( a.length != b.length ) return null;

    byte[] result = new byte[ a.length ];
    for ( int ii = 0; ii < result.length; ii++ )
      result[ii] = (byte)(a[ii] ^ b[ii]);

    return result;
  }

  public static byte[] concat( byte[] a, byte b )
  {
    byte[] ba = new byte[] { b };
    return concat( a, ba );
  }

  public static byte[] concat( byte[] a, byte[] b )
  {
    if (a == null) return b;
    if (b == null) return a;

    byte[] result = new byte[ a.length + b.length ];
    System.arraycopy( a, 0, result, 0, a.length );
    System.arraycopy( b, 0, result, a.length, b.length );
    return result;
  }

  public static byte[] prepend( byte b, byte[] ba )
  {
    byte[] result = new byte[ ba.length + 1 ];

    result[0] = b;
    System.arraycopy( ba, 0, result, 1, ba.length );

    return result;
  }

  public static byte[] dropFirstByte( byte[] a )
  {
    if (null == a || 0 == a.length) return a;
    return Arrays.copyOfRange( a, 1, a.length );
  }

  public static void main( String[] args )
  {
    byte[] a   = new byte[] { (byte)0x00, (byte)0x01, (byte)0x02, (byte)0x03 };
    byte[] b   = new byte[] { (byte)0x04, (byte)0x05, (byte)0x06, (byte)0x07 };
    byte[] exp = new byte[] { (byte)0x04, (byte)0x04, (byte)0x04, (byte)0x04 };

    byte[] act = xor( a, b );

    for (int ii = 0; ii < a.length; ii++)
      if (act[ii] != exp[ii])
        System.out.println( "FAIL: exp[" + ii + "]" );

    byte[] cat = concat( a, b );
    if (cat.length != (a.length + b.length))
    {
      System.out.println(
        "FAIL: " + a.length + " + " + b.length + " is " + cat.length + "?" );
      System.exit( 1 );
    }

    if ( cat[cat.length - 1] != (byte)0x07 )
    {
      System.out.println( "FAIL last byte of cat result check" );
      System.exit( 1 );
    }

    byte hdr = (byte)0xFF;
    byte[] pp = prepend( hdr, a );
    if ( pp[0] == (byte)0xFF && pp[1] == a[0] )
      System.out.println( "prepend: PASS" );
    else
      System.out.println( "prepend: FAIL" );


    byte[] lopped = dropFirstByte( a );
    if ( (byte)0x01 == lopped[0] )
      System.out.println( "dropFirstByte: PASS" );
    else
      System.out.println( "dropFirstByte: FAIL" );

    System.exit( 0 );
  }
}

